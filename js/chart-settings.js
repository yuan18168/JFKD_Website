...
(async function () {
  ...
  const gFontSize = document.getElementById("gFontSize");
  ...
  function fillForm(values, els) {
    ...
    els.fontSize.value = values.fontSize;
  ...
  const globalEls = { yMin: gYMin, yMax: gYMax, xCount: gXCount, xCountLabel: gXCountLabel, showPointLabels: gShowPointLabels, fontSize: gFontSize };
      ...
      <div class="card" style="margin-bottom:14px;" data-student-card="${s.id}">
        <div class="flex-between" style="margin-bottom:12px;">
          <div style="display:flex; align-items:center; gap:10px;">
            <span style="width:22px;height:22px;border-radius:50%;background:${s.color || "#4f7cff"};display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#08122e;">${(s.name || "?").slice(0, 1)}</span>
        ...
        <div data-custom-fields="${s.id}" style="display:none;">
          ...
          <div class="grid grid-cols-2" style="margin-top:14px;">
            ...
            <div>
              <label>字體大小</label>
              <select data-field="fontSize" data-student="${s.id}">
        ...
        <div style="margin-top:16px;">
          ...
          <span class="text-faint" style="margin-left:10px; font-size:12px;" data-save-msg="${s.id}"></span>
        </div>
      </div>`
    )
    .join("");
  ...
  students.forEach((s) => {
    ...
    const els = {
      ...
      fontSize: overridesEl.querySelector(`[data-field="fontSize"][data-student="${s.id}"]`),
    };
